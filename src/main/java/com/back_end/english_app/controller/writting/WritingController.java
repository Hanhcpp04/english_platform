...
public class WritingController {
     ...
     */
    @PostMapping("/submit")
    public APIResponse<GradingResultResponse> submitWriting(
            @Valid @RequestBody SubmitWritingRequest request,
            @RequestParam("userId") Long userId) {
        try {
            log.info("Received writing submission from user {}", userId);
            GradingResultResponse result = writingService.submitAndGradeWriting(request, userId);
            return APIResponse.success(result);
        } catch (RuntimeException e) {
            log.error("Error submitting writing: {}", e.getMessage());
            return APIResponse.error(e.getMessage());
        } catch (Exception e) {
            log.error("Unexpected error submitting writing: {}", e.getMessage(), e);
            return APIResponse.error("Failed to grade writing: " + e.getMessage());
        }
    }
}
